...
import { blueprintTheme, DimH, DimV, PadGuide, Selection } from "@/components/blueprints/parts";
...
function useAnatomy() {
  ...
  if (!context) throw new Error("useAnatomy must be used within AnatomyProvider");
  return context;
}

// --- Hooks ---
function useSpotlight(partIds: string | string[], options?: { isInteraction?: boolean; defaultOpacity?: number }) {
  ...
  const opacityClass = options?.isInteraction 
    ? (isHovered ? "opacity-100" : isOthersHovered ? "opacity-20" : "opacity-40")
    : (isHovered ? "opacity-100" : isOthersHovered ? "opacity-30" : `opacity-${defaultOp}`);
...
// --- Components ---
function Badge({ part, label, isAccent = false, className }: any) {
...
function AnatomyTrack({ theme }: { theme: any }) {
  ...
  return (
    <rect
      ...
      fill={hovered === "root" || hovered === "track" ? "currentColor" : "transparent"}
...
function AnatomyThumb({ theme }: { theme: any }) {
  const { hovered, setHovered } = useAnatomy();
...
function AnatomyTexts() {
  ...
  return (
    <>
      <text
        x={24} y={73.5} textAnchor="start" fontSize={14} fontWeight={500} fontFamily="var(--font-sans)"
      ...
      <text
        x={196} y={73.5} textAnchor="end" fontSize={14} fontWeight={500} fontFamily="var(--font-sans)"
...
function AnatomyBackground() {
  ...
  return (
    ...
    >
      <PadGuide
        x={24} y={60} w={172} h={18}
        offset={0.8} boxX={10} boxY={50} boxW={200} boxH={38} boxRx={8} clipOffset={0.8}
      />
      <g fontSize={9} fontFamily="var(--font-mono)" fill="currentColor" textAnchor="middle" opacity={0.5}>
        <text x={17} y={72}>14</text>
        <text x={203} y={72}>14</text>
        <text x={130} y={57.5}>10</text>
        <text x={130} y={86.5}>10</text>
      </g>
      <text x={10} y={43} fontSize={10} fontFamily="var(--font-mono)" fill="currentColor" opacity={0.5} fontWeight={500}>
        r8
      </text>
      <Selection x={10} y={50} w={200} h={38} />
      <DimH x1={10} x2={210} y={35} label="200" />
      <DimV x={225} y1={50} y2={88} label="38" labelXOffset={5} labelAnchor="start" />
    </g>
  );
}
  ...
  return (
    <>
      <rect
        ...
        style={{ pointerEvents: "all", fillOpacity: hovered === "interaction" ? 0.1 : 0, ...spotlight.style }}
        onMouseEnter={() => setHovered("interaction")}
        onMouseLeave={() => setHovered(null)}
      />
      <line
        x1={184} y1={109} x2={310} y2={109}
        strokeWidth={1}
        strokeDasharray={hovered === "interaction" ? "none" : "2 2"}
...
function AnatomyBadges() {
  return (
    <>
      <foreignObject x={70} y={45} width={100} height={20} className="overflow-visible pointer-events-none">
        <Badge part="root" label="Slider.Root" className="items-end justify-start" />
      </foreignObject>
      <foreignObject x={70} y={20} width={100} height={20} className="overflow-visible pointer-events-none">
        <Badge part="indicator" label="Slider.Indicator" className="items-end justify-center" />
      </foreignObject>
      <foreignObject x={112} y={145} width={100} height={20} className="overflow-visible pointer-events-none">
        <Badge part="thumb" label="Slider.Thumb" className="items-start justify-center" />
      </foreignObject>
      <foreignObject x={220} y={175} width={100} height={20} className="overflow-visible pointer-events-none">
        <Badge part="track" label="Slider.Track" className="items-start justify-start" />
      </foreignObject>
      <foreignObject x={310} y={99} width={160} height={20} className="overflow-visible pointer-events-none">
        <Badge part="interaction" label="Interaction Zone (&ge;44px)" isAccent className="items-center justify-start" />
      </foreignObject>
    </>
  );
}
  ...
  return (
    <AnatomyContext.Provider value={{ hovered, setHovered }}>
      <div className="flex w-full items-center justify-center overflow-visible py-10">
        ...
        >
          <defs>
            <pattern id="bp-anatomy-hatch" patternUnits="userSpaceOnUse" width="4" height="4" patternTransform="rotate(45)">
              <rect width="4" height="4" className="fill-background" />
              <line x1="0" y1="0" x2="0" y2="4" stroke="currentColor" strokeWidth="0.75" className="opacity-40" />
            </pattern>
